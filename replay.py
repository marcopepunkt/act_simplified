...
def main():
    # play cam video
    data_file = '/home/aidara/augmented_imitation_learning/training_data/pos_test/episode_4.hdf5'
    ...
    for i,q in enumerate(qpos):
        ...
        # safety_offset = 1e-2
        # if np.any(np.isclose(joint_state, low, atol=safety_offset)) or np.any(np.isclose(joint_state, high, atol=safety_offset)):
        #     print("Warning: action is close to joint limit")
        # joint_state = np.clip(joint_state, low + safety_offset, high - safety_offset)
        ...
        time.sleep(10/30)   #move_to_joint_position(robot,q.tolist(), 0.5)
